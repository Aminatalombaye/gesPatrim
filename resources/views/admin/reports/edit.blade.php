    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.reports.update", [$report->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('report_date') ? 'is-invalid' : '' }}" type="text" name="report_date" id="report_date" value="{{ old('report_date', $report->report_date) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('projects') ? 'is-invalid' : '' }}" name="projects[]" id="projects" multiple required>
                    @foreach($projects as $id => $project)
                        <option value="{{ $id }}" {{ (in_array($id, old('projects', [])) || $report->projects->contains($id)) ? 'selected' : '' }}>{{ $project }}</option>
                    @endforeach
                </select>
