    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.bons.update", [$bon->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('bons') ? 'is-invalid' : '' }}" name="bons[]" id="bons" multiple required>
                    @foreach($bons as $id => $bon)
                        <option value="{{ $id }}" {{ (in_array($id, old('bons', [])) || $bon->bons->contains($id)) ? 'selected' : '' }}>{{ $bon }}</option>
                    @endforeach
                </select>
                @if($errors->has('bons'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('date_livraison') ? 'is-invalid' : '' }}" type="text" name="date_livraison" id="date_livraison" value="{{ old('date_livraison', $bon->date_livraison) }}" required>
