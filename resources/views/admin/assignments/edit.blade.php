    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.assignments.update", [$assignment->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('type_attribution') ? 'is-invalid' : '' }}" type="text" name="type_attribution" id="type_attribution" value="{{ old('type_attribution', $assignment->type_attribution) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('asset') ? 'is-invalid' : '' }}" type="text" name="asset" id="asset" value="{{ old('asset', $assignment->asset) }}">
                @if($errors->has('asset'))
                    <div class="invalid-feedback">
                        {{ $errors->first('asset') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('user') ? 'is-invalid' : '' }}" type="text" name="user" id="user" value="{{ old('user', $assignment->user) }}">
                @if($errors->has('user'))
                    <div class="invalid-feedback">
                        {{ $errors->first('user') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('attribution') ? 'is-invalid' : '' }}" name="attribution_id" id="attribution_id">
                    @foreach($attributions as $id => $entry)
                        <option value="{{ $id }}" {{ (old('attribution_id') ? old('attribution_id') : $assignment->attribution->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
