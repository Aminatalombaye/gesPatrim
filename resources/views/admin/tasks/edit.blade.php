...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.tasks.update", [$task->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="equipements">{{ trans('cruds.task.fields.equipement') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select class="form-control select2 {{ $errors->has('equipements') ? 'is-invalid' : '' }}" name="equipements[]" id="equipements" multiple>
                    @foreach($equipements as $id => $equipement)
                        <option value="{{ $id }}" {{ (in_array($id, old('equipements', [])) || $task->equipements->contains($id)) ? 'selected' : '' }}>{{ $equipement }}</option>
                    @endforeach
                </select>
                @if($errors->has('equipements'))
                    <div class="invalid-feedback">
                        {{ $errors->first('equipements') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.task.fields.equipement_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="scheduled_date">{{ trans('cruds.task.fields.scheduled_date') }}</label>
                <input class="form-control date {{ $errors->has('scheduled_date') ? 'is-invalid' : '' }}" type="text" name="scheduled_date" id="scheduled_date" value="{{ old('scheduled_date', $task->scheduled_date) }}" required>
                @if($errors->has('scheduled_date'))
                    <div class="invalid-feedback">
                        {{ $errors->first('scheduled_date') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.task.fields.scheduled_date_helper') }}</span>
            </div>
            <div class="form-group">
