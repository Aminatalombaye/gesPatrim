...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.barcode.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.barcodes.update", [$barcode->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('code_barres') ? 'is-invalid' : '' }}" name="code_barres[]" id="code_barres" multiple required>
                    @foreach($code_barres as $id => $code_barre)
                        <option value="{{ $id }}" {{ (in_array($id, old('code_barres', [])) || $barcode->code_barres->contains($id)) ? 'selected' : '' }}>{{ $code_barre }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $barcode->description) }}">
