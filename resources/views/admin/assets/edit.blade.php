...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.assets.update", [$asset->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label>{{ trans('cruds.asset.fields.supplier') }}</label>
                <select class="form-control {{ $errors->has('supplier') ? 'is-invalid' : '' }}" name="supplier" id="supplier">
                    <option value disabled {{ old('supplier', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Models\Asset::SUPPLIER_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('supplier', $asset->supplier) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
